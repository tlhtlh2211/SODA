...
def layer_masking(z, mask_prob):
    ...
    Parameters:
    - z: The input latent tensor with tuple of sub-latent of size (batch_size, dim).
...
def inverted_beta_schedule(timesteps, s = 0.008):
    """
    inverted schedule
    ...
    assert 0 <= t <= 1 
    alphas_cumprod = torch.cos((t + s) / (1 + s) * math.pi * 0.5) ** 2
    alphas_cumprod = 2 - 2*t - alphas_cumprod 
    alphas_cumprod = alphas_cumprod / alphas_cumprod[0]
    betas = 1 - (alphas_cumprod[1:] / alphas_cumprod[:-1])
    return torch.clip(betas, 0, 0.999)
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.model = model
        #TODO: Add encoder to other __init__
    ...
    def p_losses(self, x_start, x_latent, t, noise = None, offset_noise_strength = None):
        b, c, h, w = x_start.shape

        noise = default(noise, lambda: torch.randn_like(x_start))
        
        #TODO: Add x_latent in dataset and trainer loop
        latent = self.encoder(x_latent)
        ...
        # TODO: Add layer modulation and masking to Unet
        model_out = self.model(x, latent, t, x_self_cond)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        with torch.inference_mode():
                            ...
                            sample_latent = torch.stack([sample[1] for sample in Subset(self.ds, subset_indices)], dim=0).float().to(device)
    ...
    def linear_probe(self):
        ...
        with torch.no_grad():
            for images, augmented_images, labels in tqdm(DataLoader(self.ds, batch_size=100)):
                features = self.model.encoder(augmented_images.to(self.device))
